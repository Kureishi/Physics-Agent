...
Field ownership by stage (who fills each field in):
    ...
    Stage 3: checks_run, checks_failed, check_details, final_confidence
        (initial estimate)
    Stage 4 -- self-correction: error_type, revision_count,
        revision_history, resolution_status, final_answer, time_to_solve_ms
    Stage 8 -- curriculum (this implementation): source ("user" or
        "curriculum"), curriculum_target (the weak-area signal that
        motivated this problem, if source == "curriculum")
...
class Trace:
    ...
    # --- Stage 8: curriculum -------------------------------------------------
    # "user" for a problem someone submitted directly, "curriculum" for one
    # generated by CurriculumRunner to target a Stage 7 weak-area signal.
    # curriculum_target holds that signal, for traceability.
    source: str = "user"
    curriculum_target: Optional[Dict[str, Any]] = None
...
class EpisodicMemory:
    ...
    def query_by_source(self, source: str) -> List[Trace]:
        """Stage 8: e.g. query_by_source("curriculum") to see only
        self-generated practice problems, separate from ones a person
        submitted directly."""
        return [t for t in self.read_all() if t.source == source]
